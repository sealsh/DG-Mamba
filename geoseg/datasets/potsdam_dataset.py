...
class PotsdamDataset(Dataset):
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        ...
        # --- 在这里添加过滤代码 ---
        # 只保留以 .png 结尾且不以 . 开头的文件
        img_filename_list = [f for f in img_filename_list if f.endswith(self.img_suffix) and not f.startswith('.')]
        mask_filename_list = [f for f in mask_filename_list if f.endswith(self.mask_suffix) and not f.startswith('.')]
        # --- 添加结束 ---
